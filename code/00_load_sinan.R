...
# add population data & municipality names
...
sinan_xpert <- left_join(sinan_xpert, population %>% group_by(id_state) %>% summarize(state_pop = sum(pop_total_2010)), by = c("sg_uf" = "id_state"))

## municipality name
sinan_xpert <- left_join(sinan_xpert, population %>% select(id_municip, municip_name), by = "id_municip") 
...
source(here::here("code/00_add_health_unit_type.R"))

sinan_xpert <- merged_health_unit %>% select(!c("state", "municip_cod", "cnes", "uni_tp_cod", "uni_tp", "year", "first_level", "second_level_tb", "third_level_tb", "second_level", "third_level", "others"))
