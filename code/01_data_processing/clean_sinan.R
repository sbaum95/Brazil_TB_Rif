...
## Clean age 
  ...
  # See how many 0-4 YOs who have schooling; Individuals > 110
  # sinan_tmp <- sinan_tmp %>%
  #   mutate(
  #     age = if_else((age_idade <= 4 & cs_escol_n %in% c("4", "5", "6", "7", "8")), NA, age_idade),
  #     age_flag = if_else((age_idade <= 4 & cs_escol_n %in% c("4", "5", "6", "7", "8")), "0-4 w/ school",
  #       if_else(age_idade > 110, "> 110", "age_idade")
  #     )
  #   )
...
sinan_tmp <- sinan_tmp %>%
    mutate(
      ...
      # Age category
      age_cat = if_else(is.na(age_cat), "missing", age_cat) %>% 
        as.factor() %>% 
        relevel(ref = "25-34"),
      # Health Unit
      health_unit = if_else(is.na(health_unit), "missing", health_unit) %>% 
        as.factor(),
      # Race
      ...
      # Tested defined as having a conclusive resistance result (either resistant or sensitive)
      tested = if_else(test_molec %in% c("1", "2"), "tested", "not tested") %>% 
        as.factor(),
      # Xpert result
