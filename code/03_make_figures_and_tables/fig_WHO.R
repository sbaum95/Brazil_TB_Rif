                                ...
                                if_else(year == 2018, 18, 
                                        if_else(year == 2019, 22, 
                                                if_else(year == 2020, 26, 
                                                        if_else(year == 2021, 30, 
                                                                if_else(year == 2022, 34)))))))))




who_data_new <- compiled_results[["nat_yr"]] %>% 
  filter(model == "sp_2015" & case_type == "new") %>% 
...
who_data_prev <- compiled_results[["nat_yr"]] %>% 
  filter(model == "sp_2015" & case_type == "prev") %>% 
  select(year, total_TB_cases) %>% 
...
data_total <- data.frame(
  ...
  cdr = compiled_results[["nat_yr"]] %>%
    filter(model == "sp_2015") %>%
    group_by(year) %>%
    summarize(cases = if_else(year <= "2017", sum(fitted_RR)/0.87, 
                              if_else(year %in% c("2018", "2019"), sum(fitted_RR)/0.89, 
                                      if_else(year == "2020", sum(fitted_RR)/0.78, 
                                              if_else(year == "2021", sum(fitted_RR)/0.76, 
                                                      if_else(year == "2022", sum(fitted_RR)/0.83)))))) %>% 
    unique(), 
...
fig_total_inc <- ggplot() +
  ...
  scale_x_continuous(expand = c(0, 0), 
                     breaks = c(2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022)) + 
  ggtitle("New") + 
  ...
  geom_line(data = data_total,
            aes(x = diag_yr, y = total.observed/350, linetype = "Total number of observed RR-TB cases")) + 
