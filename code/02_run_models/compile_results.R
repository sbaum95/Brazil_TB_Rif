...
aggregate_fitted_values <- function(model_name, agg_level) {
  ...
  if (agg_level == "state_qrt") {
    ...
    fitted <- fitted %>%
      separate(colnames(.)[1], into = c("state", "year"), sep = ",", remove = TRUE) %>%
      mutate(
        model = model_name,
...
get_observed <- function(model_name, agg_level) {
  ...
  if (agg_level == "state_qrt") {
    ...
    observed <- observed %>%
      separate(colnames(.)[1], into = c("state_nm", "state", "year"), sep = ",", remove = TRUE) %>%
      mutate(
        model = model_name,
        year = as.numeric(year)
      )
